    ...
    try {
      ...
      if (promo) {
        ...
        await supabase.from('notifications').insert({
          ...
          message: `Your ${accountType} promotion "${promoName}" has been removed. Your account has been downgraded to free tier.`,
          type: 'system',
  ...
  useEffect(() => {
    ...
    const unsubscribe = useAuthStore.subscribe(
      ...
      (newProfile, oldProfile) => {
        if (newProfile?.account_type !== oldProfile?.account_type) {
          fetchUserPromos();
        }
      }
    );
