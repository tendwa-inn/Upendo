...
const Stories: React.FC = () => {
  ...
  const handleAddStory = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { data: uploadData, error: uploadError } = await supabase.storage.from('stories').upload(fileName, file);
    ...
    const publicUrl = `${supabaseUrl}/storage/v1/object/public/stories/${uploadData.path}`;
