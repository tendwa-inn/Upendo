...
export const wordFilterService = {
  ...
  async getCurrentUserSuspension(userId: string) {
  ...
  async updateFilteredWord(id: number, newWord: string) {
    const { error } = await supabase
      .from('word_filter')
      .update({ word: newWord })
      .eq('id', id);
  ...
  async takeUserAction(userId: string, actionType: 'warning' | 'suspension' | 'ban', adminId: string, reason?: string, expiresAt?: string) {
    const { data, error } = await supabase
      .from('user_actions')
      .insert([{
        user_id: userId,
