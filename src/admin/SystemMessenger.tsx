...
const SystemMessenger: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      if (attachedPhoto) {
        photoUrl = await fileUploadService.upload(attachedPhoto, 'messages');
      }
      ...
      if (messageTypes.includes('notification')) {
        if (finalTarget === 'all') {
          ...
          for (const user of allUsers) {
            await notificationService.sendSystemMessage(user.id, title, message);
          }
        } else {
          await notificationService.sendSystemMessage(finalTarget, title, message);
        }
      }
